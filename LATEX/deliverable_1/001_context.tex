...
Computed Tomography scans of head-and-neck cancer patients with associated survival data. 
...
\begin{description}
  \item[Event \( E \)] It can be death (\( E = 1 \)) or otherwise it can be recovery or withdrawal
  (\( E = 0 \)).
  \item[Time \( T \)] Time from the beginning of the observation period until an event, the end 
  of the study or withdrawal from the study. 
  \item[Baseline data \( x \)] Available data to predict the survival time.
  \item[Censoring] If a subject does not have an event during the observation time they are
  described as censored. This means that nothing is known about the subject after the censoring.
\end{description}
...
model is performing better than another, is the \emph{Concordance Index} (C-index). To obtain this 
indicator, pairs are generated to compare the survival time. If 
\( T_i > T_j \) and \( \hat{T}_i > \hat{T}_j \) then it's counted as a good prediction, otherwise
it's counted as a bad prediction, note that \( T_i = \hat{T}_i \) it's not a required condition. 
The number of good predictions is divided by the total
number of predictions. This index is 1 for a perfect data fit and 0.5 for a random model.
~\cite{medical:ranking-ci}

\[
  CI = \frac{\text{Good predictions}}{\text{Total predictions}}
\]
