...
These are called hidden units because the values of the \( \bm{a} \) are not directly 
observed. In general, more than one hidden unit is used to fit more complex features.
...
\begin{align*}
  z_i^{[l]} &= b_i^{[l]} + \sum_{j = 1}^{n^{[l]}} w_{ij}^{[l]} \cdot a_j^{[l - 1]} \\
  a_i^{[l]} &= g^{[l]}(z_i^{[l]})
\end{align*}
...
To fit a neural network model the cost function \( C(\bm{y}, \hat{\bm{y}}) \) needs to be
minimized. To do so, gradient descent is used so, each weight gets updated by the partial
derivative 
%TODO: Finish this section
