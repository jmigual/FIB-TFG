...
by a graph (or network) diagram as seen in \autoref{fig:neural_network}. It is defined by
multiple layers and each layer has multiple units. The number of units in layer \( l \) 
is defined by  \( n^{[l]} \).

Each unit has an activation (output) computed in two steps, the linear and the non-linear
step, \( z \) and \( a \) respectively.
...
The non-linear step is defined by applying a
non-linear function, called activation, (\( g^{[l]} \)) to the linear part. 
Multiple activation functions can be seen in \autoref{fig:activation_functions}. 

So, the whole activation for unit \( i \) of layer \( l \) is as follows:
\begin{align*}
  z_i^{[l]} &= \sum_{j = 1}^{n^{[l]}} w_{ij}^{[l]} \cdot a_j^{[l - 1]} \\
  a_i^{[l]} &= g^{[l]}(z_i^{[l]})
\end{align*}

A matrix notation can be used when applying this to all the layer's units.
% \begin{align*}
...
\begin{figure}
  ...
  \caption[Neural network graph]{
    ...
    Each arrow represents a weight $w_{ij}^{[l]}$ from two units.
