...
in this case, there is \gls{censored} data some data cannot be used always. Also, we want to
obtain the survival function \( S(t) \) or the hazard function \( \lambada(t) \) which is not 
directly provided in the dataset, as our output values are the \gls{event} 
\glssymbol{event} and the \gls{time} \glssymbol{time}.

DeepSurv is a machine learning package, with a published paper, using a survival model 
