...
The distance comparison used for this siamese network is defined comparing the absolute distance
of the sister's output features and then applying the logistic function into it to get an
output value \( d \in [0, 1] \). This way if it's 1 it means \( T_B > T_A \).
\begin{align*}
...
So the model's cost function, adding the regularization loss, is:
\[
  C(\bm{y}, \hat{\bm{y}}) = \mathcal{L}(\bm{y}, \hat{\bm{y}}) + 
  ||w||_2
\]

The results with this model are still not valid and this task has been scheduled for later as
the training times are really high. Once there is a basic working model with scalar features
only this model will be finished and the results obtained.
...
The a summary of the results using 4-CV can be seen at \autoref{tab:scalar-4CV}. It can be
...
Even though it seems that these results may seem similar to the state-of-the art (~0.65) 
they 
