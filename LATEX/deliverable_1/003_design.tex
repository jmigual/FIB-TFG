...
The results with this model have been obtained using 4-CV and \gls{LOOCV}.


% TODO: Add figures
The results obtained with \gls{LOOCV} are shown in FIGURE. To train this model one element has
been removed from the dataset and the model has been trained with the rest of the elements 
(all minus one). As it can be seen the distribution for the uncensored data is quite similar from
the rest of the dataset. The final \gls{CI} is NUMBER and the mean \gls{CI} is NUMBER.
...
To speed up the testing process a scalar only siamese network. With this approach the training
time was reduced from around 5 hours to 1 minute for one training step. 

To improve the performance Dropout was used, by using dropout some of the model activations 
are zeroed and then scaled according to the dropout probability. This way, the networks'
neurons do not have all the information and thus it must generalize. Dropout is used as a 
way of regularization. Moreover, regularization is applied to the model too by adding the 
l2 norm of the weights.

The loss and cost functions are the same as the ones used in \autoref{sec:shallow-siamese}
...
A summary of the results using 4-CV can be seen at \autoref{tab:scalar-4CV}. It can be
observed that the number of pairs is not constant through all the folds. This is caused by
censoring, since not all members can form a pair with all the other ones some possibilities 
have more available pairs than others. From the results it can be seen that the final test
\gls{CI} is 0.624 and the final mixed \gls{CI} is 0.789.

Even though it seems that these results may seem similar to the state-of-the art (0.628) for 
the test set, the improvement in this project is in the mixed pairs. As the main goal of the
project is to predict survival, the final created method to predict survival will be to 
compare a new patient against the whole training set and then, having all the comparisons and
the survival time of the train set patients, a prediction of the new patient's survival time
can be made.
...
\begin{table}
  ...
  \begin{tabular}{|c||c|c|c||c|c|c|}
    ...
    \hline
    \textbf{Fold} & \textbf{Train} & \textbf{Test} & \textbf{Mixed} & 
    \textbf{Train} & \textbf{Test} & \textbf{Mixed} \\
    \hhline{=======}
    0 & 82.518 & 9.254 & 55.512 & 0.959 & 0.619 & 0.784 \\
    \hline
    1 & 82.958 & 9.150 & 55.176 & 0.960 & 0.637 & 0.798 \\
    \hline
    2 & 83.416 & 9.004 & 54.864 & 0.953 & 0.622 & 0.791 \\
    \hline
    3 & 82.396 & 9.312 & 55.576 & 0.954 & 0.618 & 0.784 \\
    \hhline{=======}
    \textbf{Total} & 331.288 & 36.720 & 221.128 & 0.957 & 0.624 & 0.789 \\
    \hline
  \end{tabular}

  \caption{Results for scalar only model using 4-CV \label{tab:scalar-4CV}}
\end{table}
