...
% C-index explanation
To the standard performance measure to compare if a survival model is performing better 
than another is the \emph{Concordance Index} (C-index). This index is 1 for a perfect data
fit and 0.5 for a random model. Also, another comparison chart is the ROC
curve which compares the \emph{False Positive Rate} against the \emph{True Positive Rate},
see \autoref{fig:ROC-curve}. Also another comparison value is the \emph{concordance index}. 
Usually the C-index is seen as the area under the ROC curve.
...
When working with Survival Prediction models the type of data usually has always the same form.
Survival data has three elements: a patient's baseline data \( x \), a failure event time 
