...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep, scale=.8]
        ...
        \foreach \y in {1,...,3} {
            \node[input neuron] (I-\y) at (0,{1 -(\y - 1)}) {$x^{(\y)}$};
        }
        % Draw the hidden layer nodes
        \foreach \y in {1,...,4} {
            \node[hidden neuron] (H-\y) at (\layersep,{2.25 - 1.5*(\y -1)}) {$a^{(\y)[1]}$};
        }
        
        % Draw the output layer node
        \node[output neuron,pin={[pin edge={->}]right:Output}] (O) at ({2*\layersep, 0}) {$a^{(1)[2]}$};
